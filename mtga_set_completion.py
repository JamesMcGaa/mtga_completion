from constants import EXPANSION_RARITY_DISTRIBUTION, CARDS_PER_RARITY
import random
...
class mtg_set:
    def __init__(
        ...
        self.mythic_rate = mythic_rate

        if preexisting == None:
            # rarity C,U,R,M to a list of counts in range [0,4]
            self.preexisting = list(map(lambda x: [0 for _ in range(x)], rarity_distribution))
    ...
    def add_cards_per_rarity(self, rarity_index):
        ...
        if rarity_index in [0,1]:
            drawn_unique_cards = random.sample([index for index in range(self.rarity_distribution[rarity_index])], num_nonwildcards_to_open)
            ...
            non_complete = list(filter(lambda  index: self.preexisting[rarity_index][index] != 4, all_indices))
    ...
    def count_missing_at_rarity(self, rarity_index):
        return 4*self.rarity_distribution[rarity_index] - sum(self.preexisting[rarity_index])
        ...
        for rarity_index in range(4):
            if self.wildcards[rarity_index] > 0:
                ...
                #complete set at rarity
                    ...
                    self.preexisting[rarity_index] = [4 for _ in range(self.rarity_distribution[rarity_index])]
...
def experiment(num_trials):
    ...
    for t in range(num_trials):
        ...
        running_count += mtg_set(EXPANSION_RARITY_DISTRIBUTION['ZNR'], 1/7.4).complete_set()
    return running_count / num_trials
